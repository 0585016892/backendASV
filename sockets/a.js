...
function chatSocket(io) {
  ...
  function getBotReply(message, callback) {
    ...
    console.log("🤖 [BOT DEBUG] Nhận tin nhắn:", lowerMsg);
    ...
    db.query(replySql, [lowerMsg], (err, results) => {
      ...
      if (results.length > 0) {
        console.log("🤖 [BOT MATCH] Keyword trùng:", results[0].reply);
      ...
      db.query(productSql, [lowerMsg], (err, prods) => {
        ...
        if (prods.length === 0) {
          console.log("🤖 [BOT INFO] Không tìm thấy sản phẩm phù hợp");
        ...
        console.log("🤖 [BOT PRODUCT] Trả về:", reply);
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("register", (userId) => {
      if (!userId) return;
      console.log(`👤 [REGISTER] User ${userId} vừa kết nối với socket ${socket.id}`);
      ...
      if (userId === "admin") {
        console.log("🧾 [ADMIN] Kiểm tra tin nhắn chưa đọc...");
        const unreadSql = `
        ...
        db.query(unreadSql, (err, rows) => {
          if (!err && rows.length > 0) {
            const latest = rows[0];
            console.log(`📨 [ADMIN NOTICE] Có ${rows.length} tin chưa đọc.`);
    ...
    socket.on("send_private_message", (data, callback) => {
      ...
      console.log(`💬 [SEND MESSAGE] ${sender} → ${receiver}: "${content}"`);
      ...
      if (receiver === "admin" && !isAdminOnline) {
        console.log("🤖 [BOT MODE] Admin đang offline → bot phản hồi");
        getBotReply(content, (reply) => {
          ...
          console.log("🤖 [BOT REPLY] Đã gửi phản hồi:", reply.content);
